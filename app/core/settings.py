...
class Settings:
    ...
    brevo_sender_name: str = "BheemBhai"
...
def get_settings() -> Settings:
    ...
    brevo_sender_name = os.getenv("BREVO_SENDER_NAME") or os.getenv(
        "EMAIL_BREVO_SENDER_NAME", "BheemBhai"
    )

    return Settings(
        app_name=os.getenv("BHEEMBHAI_APP_NAME", "BheemBhai"),
        environment=os.getenv("BHEEMBHAI_ENVIRONMENT", "development"),
        version=os.getenv("BHEEMBHAI_APP_VERSION", "0.1.0"),
        api_version=os.getenv("BHEEMBHAI_API_VERSION", "v1"),
        public_base_url=os.getenv("BHEEMBHAI_PUBLIC_BASE_URL", "http://localhost:8000"),
        secret_key=os.getenv("BHEEMBHAI_SECRET_KEY", "change-me"),
        database_url=os.getenv(
            "DATABASE_URL", "sqlite+pysqlite:///./.local/beembhai.db"
        ),
        secure_storage_key=os.getenv("BHEEMBHAI_SECURE_STORAGE_KEY", "change-me-secure-storage"),
